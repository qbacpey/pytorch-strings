...
def string_tensors(tpch_data: Dict[str, List[str]], operators: List[TestOperator], encoder: StringEncoder, device: str) -> Dict[str, StringColumnTensor]:
    if device == "cuda":
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
    
    tensor_cols: Dict[str, StringColumnTensor] = {}
    with torch.device(device):
        if device == "cuda":
            torch.cuda.empty_cache()
            torch.cuda.synchronize()
        col_names = [op.col_name for op in operators]
        tensor_cols = {col_name: encoder.encode(tpch_data[col_name]) for col_name in col_names}

    return tensor_cols

def string_processing(benchmark: pytest_benchmark.fixture.BenchmarkFixture, string_tensors: Dict[str, StringColumnTensor], operators: List[TestOperator], device: str):
    with torch.device(device):
        cols = string_tensors
...
class TestStringColumnTensor:
    ...
    @pytest.mark.parametrize("encoder", [PlainEncoder(), DictionaryEncoder()], ids=lambda encoder: encoder.__class__.__name__)
    @pytest.mark.parametrize("device", ["cpu", "cuda"])
    @pytest.mark.parametrize("operators", [
        [FilterScan('l_shipmode', PredicateEq('AIR'))],
    ])
    def test_string_processing(self, benchmark, string_tensors, operators: List[TestOperator], encoder: StringEncoder, scale: float, device: str):
        print(f"Testing string query processing with encoder {encoder.__class__.__name__} on scale {scale} and device {device}...")
        string_processing(benchmark, string_tensors, operators, device)
