...
# source("00_linear_GB_CPU_vs_GPU_mssb.r")
...
# --- 2. Plotting Function (Linear Scale for MSSB) ---
...
create_throughput_plot <- function(df) {
  ...
  p <- ggplot(df, aes(x = `param:total_count`, y = throughput_gb_per_sec, color = Device, group = Device)) +
  ...
  # --- MODIFIED: Use a log scale for the x-axis with scientific notation ---
  p <- p + scale_x_log10(
  ...
  p <- p + labs(
      ...
      x = "Number of Elements (Total Count)",
      y = "Throughput (GB/s)",
      linetype = "Line Type"
    ) +
...
if (nrow(plot_data) > 0) {
  ...
  output_filename <- file.path(output_dir, paste0("00_linear_GB_MSSB_Faceted_", descriptive_pred_name, filename_suffix_mask, filename_suffix_compile, ".png"))
