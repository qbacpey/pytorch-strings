...
# This script generates a plot showing Plain and Unsorted Dictionary encodings.
...
if (length(args) != 4) {
  stop("Usage: Rscript 00_linear_GB_CPU_vs_GPU_tpch_plain_unsorted.r <input_file> <use_mask> <use_torch_compile> <predicate>", call. = FALSE)
}
...
# --- MODIFIED: Added Unsorted Dictionary encodings ---
encoding_name_map <- c(
...
if (nrow(plot_data) > 0) {
  ...
  p <- create_throughput_plot(plot_data) +
    ...
    labs(
      title = paste("TPC-H Throughput for Plain & Unsorted Encodings (Predicate: ", descriptive_pred_name, ")", sep = ""),
      subtitle = subtitle_text,
      caption = "Each panel represents a different string encoding."
    )

  # --- MODIFIED: New filename for plain & unsorted plot ---
  ...
  output_filename <- file.path(output_dir, paste0("00_linear_GB_TPCH_PlainUnsorted_", descriptive_pred_name, filename_suffix_mask, filename_suffix_compile, ".png"))
  
  ggsave(output_filename, p, width = 12, height = 8, dpi = 300, bg = BACKGROUND_STYLE)
  print(paste("Plain & Unsorted plot saved to:", output_filename))
} else {
